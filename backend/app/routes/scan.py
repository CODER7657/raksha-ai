...
def get_history(user_id: str = Depends(get_current_user)) -> list[dict]:
    ...
    try:
        ...
        # Uncaught here reaches the browser as a bare "Failed to fetch" instead
        # of a readable error (missing CORS headers on an unhandled 500) --
        # always convert DB errors into an explicit HTTPException.
